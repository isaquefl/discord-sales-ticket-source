...
class OnLoad(commands.Cog):
    ...
    async def _initialize_goatpay_webhook(self):
        """Inicia servidor HTTP para receber webhooks da GoatPay em tempo real."""
        try:
            await asyncio.sleep(3)
            ...
            async def on_goatpay_payment_approved(data: dict):
                ...
                print(f"[GoatPay Webhook] 💰 Processando pagamento aprovado: {payment_id}")
                ...
                if cart_id:
                    print(f"[GoatPay Webhook] ✅ Carrinho encontrado: {cart_id}")
                    await _handle_payment_approved(cart_id, self.bot)
                else:
                    print(f"[GoatPay Webhook] ⚠️ Carrinho não encontrado para payment_id: {payment_id}")
            ...
            print(f"[GoatPay Webhook] ❌ Erro ao iniciar servidor: {e}")
            import traceback
            traceback.print_exc()
